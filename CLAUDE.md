...
- **Per-team fuel tank capacity** — teams should have different `fuelCapacity` values rather than a shared constant. Example: McLaren best aero but only 85kg tank (forces more stops or fuel saving), Tyrrell slow but 120kg capacity (more strategy options, can run very long stints). Adds another dimension to what "best team" means and makes strategy more varied. Currently `fuelCapacity` lives on the circuit object; needs to move to (or be overridden by) a team stat, with circuit capacity as the upper bound.
...
- **Commentary colours — CRASH and OUT should be red not purple** — `tag-crash` and `tag-out` CSS classes are currently `#cc44ff` (purple). Change both to `#ff4444` (red). Purple should be reserved for fastest-lap highlights.
- **Pre-race screen — show lap count** — the race info bar or controls area should display the total number of laps for the circuit before the race starts (e.g. "75 LAPS"). Currently this is not shown until the race is running.
