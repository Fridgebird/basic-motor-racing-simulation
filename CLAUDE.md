...
- ~~**Fastest-viable compound choice at pit stops**~~ ✓ Done — `chooseNextCompound()` replaced "softest viable" heuristic with "fastest viable": for each compound that can reach the flag, compute average tyre factor via midpoint-wear approximation; pick the one with the lowest factor (ties broken in favour of harder). Aggression-based variance: fastest=soft → always soft; fastest=medium → agg×0.25 soft gamble; fastest=hard → agg×0.10 soft + agg×0.25 medium gamble. No-viable-compound branch unchanged. Soft wearMultiplier also raised 1.40→2.80 (from ~30 laps to ~15 laps on a medium circuit) so soft is genuinely a sprint gamble; hard now dominates at mid-race stops (77%) with soft only chosen in the final ~5 laps. Best strategy S→M→H averages P5.8 across a full season.
- ~~**Botched pit stop**~~ ✓ Done — probability scales with `(1 - pitCrewRating/100)²  × 0.20`; top crews near-never botch; adds 5–25s to tyre change time; netTimeLost shown in commentary.
- ~~**Flat spot from spin**~~ ✓ Done — spin events apply a quadratic wear spike (0–0.65 added wear); severe flat spots trigger the pit AI on the next lap.
- ~~**Reliability rework — era-specific failures, engine/chassis split, punctures**~~ ✓ Done — Engine and chassis reliability are now checked independently each lap (ENGINE_FAILURE_SCALE 0.028, CHASSIS_FAILURE_SCALE 0.034). Each uses its own era-aware label pool: engine labels start with 1930s mechanics (magneto, valve, crankshaft) and unlock supercharger/turbo/electronics as years advance; chassis labels include puncture as a live mechanic (85% slow puncture → tyre wear spike + `puncturePenalty` 1.10–1.20 applied as direct sector time multiplier, costs 10–20% per lap; 15% blowout → retirement as "Tyre failure"). `puncturePenalty` resets to 1.0 on next pit stop. Target rates: ~15% crash, ~28% mechanical retirement, ~57% finish. `setCurrentCircuit()` now accepts a `displayYear` arg passed from race.html, qualifying.html, and championship.js.
- ~~**Rain and wet-weather tyres**~~ ✓ Done — Markov chain weather model (DRY → DRIZZLE → RAIN → HEAVY) with four scenario types (FINE, SHOWER, CHANGEABLE, WET_START). Wet compound added: AI auto-switches when `trackWetness ≥ 0.5` (race) or `≥ 0.4` (qualifying). Track wetness evolves per lap toward state target; dries at fixed linear rate. Wet tyre grip scales with track wetness (best in soaking conditions, slow on dry track). Weather strip panel in both race and qualifying screens; weather change commentary fires on Markov state transitions.
- ~~**Wet weather qualifying**~~ ✓ Done — weather evolves driver-by-driver through the session using `tickQualiWeather(rng, lap)`; wet tyres selected when `trackWetness ≥ 0.40`. Run order strip (coloured driver pips, contrast-aware text) shown above main layout before and during the session, pips popped as each driver takes to the track. Weather forecast shown before START. Weather strip visible for all scenarios including FINE.
- ~~**Standing start penalty**~~ ✓ Done — sector 1 lap 1 applies an era-interpolated `startFactor` multiplier per car. Three outcomes: **clean** (silent, minimum penalty ×1.40→×1.10 by 2010+), **bad start** (orange `START` tag — wheelspin or bogged down, ×1.85→×1.27), **stall** (red `STALL` tag, ×2.00 in 1930 rising to ×2.40 in 1970 era peak then ×1.70 modern). Probability of bad/stall scales with `(1 − consistency/100)`. Stall peaks in 1970 era (no modern launch control); clean penalty floors at ×1.10 by 2010+. One deterministic rng call per car, replay fidelity preserved.
- ~~**Pit stop time shown in in-lap displayed time**~~ ✓ Done — pit duration (lane traversal + stationary) added to `car.lastLapTime` when a stop is executed, so the timing sheet shows a visibly slow in-lap (matching real broadcast displays) rather than hiding the time loss in the gap column only.
...
- ~~**WebP image conversion**~~ ✓ Done — converted all 59 game PNGs to WebP at quality 82 using a one-off Node/sharp script (`convert_to_webp.mjs`); 107 MB → 8.3 MB (92% reduction). Track images ~3 MB → ~300 KB; car/logo/driver sprite sheets 1.5–2 MB → 50–150 KB. All `src` and CSS `background-image` references updated across circuit, index, team, driver, drivers, qualifying HTML. `.gitignore` added to exclude `node_modules/`.
