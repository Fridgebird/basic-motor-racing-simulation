...
- **Multi-stop pit strategy planner** — `crossoverPitBenefits()` in simulation.js only models one-stop-to-flag scenarios: it finds the softest viable compound that can reach the flag and fires when the current tyre is slower than that compound at end-of-stint. This means aggressive soft starters cannot be made to deliberately undercut early for a 2-stop strategy. To fix, the AI would need to compare total estimated time for N-stop vs (N+1)-stop plans, not just ask "is there a compound that reaches the flag?". The current behaviour produces interesting drama (hard starters like Tanaka bolt for the pits early and switch to softs; soft starters stay out) but it means no driver will ever choose an early stop purely for strategic pace gain. Keep an eye on whether this becomes predictable or uninteresting.
...
- ~~**Driver and team profile pages**~~ ✓ Done — `driver.html`: portrait (individual file `portraits/driver_N.png`, falls back to initials in team colour), name, nationality, age/birth year, career totals (seasons, races, wins, podiums, titles, points), season-by-season history table with championship position and ★ for champion. `team.html`: car sprite (4×3 CSS sprite sheet), full name, nationality, career totals, season detail pane (chassis ID, engine, tyre, driver chips with portrait), prev/next season pane nav, and season history table. Each team has a distinct chassis naming formula in `getChassisId()` (e.g. Ferrosso "Tipo [season]", Hartwell "HW [digits]", FRD "Mk [Roman]"). Driver names in timing sheet are now clickable links to `driver.html`. `renderer.season` set from `race.html` so links carry the correct season param.
