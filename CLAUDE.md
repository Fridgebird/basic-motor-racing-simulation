...
- **Commentary focus filter** — a toggle cycling through Top 5 / Top 10 / All that filters commentary entries by the car's position at the time of the event. Reduces noise from midfield/backmarker pit stops and incidents. Simple to implement: store the filter level in the Renderer, re-run `_updateCommentary()` from scratch when it changes (rebuild the feed from raceLog rather than appending incrementally).
